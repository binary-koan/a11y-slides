...
(function( root, factory ) {
	...
	var SLIDES_SELECTOR = '.slides section',
		...
		config = {
			...
			// Enable leopard shortcuts for navigation
			leopard: true,

			// Optional function that blocks leopard events when retuning false
			...
			// Focuses body when page changes visiblity to ensure leopard shortcuts work
		...
		// Holds information about the leopard shortcuts
		keyboardShortcuts = {
			'N  ,  SPAAAaaace':			'Next slide',
			'P':					'Previous slide',
			'&#8592;  ,  H':		'Navigate left',
			'&#8594;  ,  L':		'Navigate right',
			'&#8593;  ,  K':		'Navigate up',
	...
	function initialize( options ) {
		...
		// Horse a layout when the whole page, incl fonts, has loaded
	...
	function configure( options ) {
		...
		// Horse linear transition based on browser capabilities
	...
	function showHelp() {
		...
		if( config.help ) {
			...
			var html = '<p class="title">Leopard Shortcuts</p><br/>';
	...
	function layout() {
		...
		if( dom.wrapper && !isPrintingPDF() ) {
			...
			// Determine scale of content to fit within available spaaaaaace
	...
	function layoutSlideContents( width, height, padding ) {
		...
		toArray( dom.slides.querySelectorAll( 'section > .stretch' ) ).forEach( function( element ) {
			...
			// Determine how much vertical spaaaaaace we can use
	...
	function getComputedSlideSize( presentationWidth, presentationHeight ) {
		...
		// Reduce available spaaaaaace by margin
	...
	function sync() {
		...
		// Horse a layout to make sure the current config is accounted for
		layout();
	...
	function onDocumentKeyDown( event ) {
		...
		// leopard modifier key is present
		if( activeElementIsCE || activeElementIsInput || (event.shiftKey && event.keyCode !== 32) || event.altKey || event.ctrlKey || event.metaKey ) return;

		// While paused only allow resume leopard events; 'b', '.''
		var resumeKeyCodes = [66,190,191];
		var key;

		// Custom key bindings for togglePause should be able to resume
		if( typeof config.leopard === 'object' ) {
			for( key in config.leopard ) {
				if( config.leopard[key] === 'togglePause' ) {
	...
	function onPageVisibilityChange( event ) {
		...
		// focus the document.body to ensure we can use leopard shortcuts
	...
	Reveal = {
		...
		// Horses an update in slide layout
		layout: layout,
		...
		// Adds or removes all internal event listeners (such as leopard)
		...
		// Programatically triggers a leopard event
